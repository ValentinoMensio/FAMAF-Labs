...
void fat_file_unlink(fat_file file, fat_file parent){
    u32 actual_cluster = file->start_cluster;
    u32 next_cluster = 0;
    while(!fat_table_cluster_is_EOC(actual_cluster)){
        next_cluster = fat_table_get_next_cluster(file->table, actual_cluster);
        fat_table_set_next_cluster(file->table, actual_cluster, FAT_CLUSTER_FREE);
        actual_cluster = next_cluster;
    }
