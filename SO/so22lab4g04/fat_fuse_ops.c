...
static void fat_fuse_log_activity(char *operation_type, fat_file file)
{
    ...
    f = fat_tree_search(vol->file_tree, BB_LOG_FILE);
    parent = fat_tree_get_parent(fat_tree_node_search(vol->file_tree, BB_LOG_FILE));
    ssize_t res = fat_file_pwrite(f, buf, message_size, f->dentry->file_size, parent);
    if (res < 0)
    {
        DEBUG("Error\n");
    }
...
static void fslog_create(void)
{
    fat_volume vol = get_fat_volume();
    fat_tree_node file = fat_tree_node_search(vol->file_tree, BB_LOG_FILE);
    if (file != NULL)
    {
        DEBUG("Archivo log ya creado\n");
    }
    else
    {
        //Busca el cluster 
        ...
        if(bb_cluster == 0){
            //Creo el directorio huerfano
            bb_init_log_dir(bb_cluster);
            DEBUG("No existe bbcluster");
            //Creo fs.log
            int fs = fat_fuse_mknod(BB_LOG_FILE, 0, 0);
            if (fs == 0){
                DEBUG("fs.log creado\n");
            }else{
                DEBUG("Error al crear fs.log");
            }
        }else{
            DEBUG("Exite bbcluster");
            //creo el directorio huerfano
            bb_init_log_dir(bb_cluster);
            //busco fs.log en el disco y lo inserto en el arbol de directorios
            fat_tree_node bb_node = fat_tree_node_search(vol->file_tree, BB_DIRNAME);   //Busco el nodo /bb en tree
            fat_file bb_file = fat_tree_get_file(bb_node);                              //Busco fat_file de /bb (datos)

            GList *bb_childrens = fat_file_read_children(bb_file);                      //Busco todos los childrens de /bb
            fat_file fs_file = g_list_nth_data(bb_childrens, 0);                        //Tomo el primer child (fs.log)

            fat_tree_insert(vol->file_tree, bb_node, fs_file);                          //Agrego fs.log al tree como hijo de /bb
        } 
    }
}
